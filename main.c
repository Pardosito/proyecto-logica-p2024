    ...
    for (int j = 0; j < rows; j++)
    {
        for (int i = 0; i < Main.num_propositions; i++)
        {
            if (j == 0)
                ...
                if ((((j-1) >> (Main.num_propositions - i - 1 )) & 1) == 0)//Bitwise operation AND with number 1 to print the values of the propositions 
                {
                    printf("|%-11s|","V");
                }
                else printf("|%-11s|","F");
...
void print_table(proposition Main) {
    for (int i = 0; i < Main.rows; i++) {           // Loop through each row
        for (int j = 0; j < Main.num_propositions; j++) { // Loop through each column
